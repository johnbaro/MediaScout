...
namespace MediaScout
{
    public class TVScout
    {
        ...
        public String ProcessDirectory(String directory)
        {
            ...
            name = IOFunctions.GetDirectoryName(directory);
            if (options.RenameFiles)
            {
                String newName = IOFunctions.GetValidName(series.SeriesName, options.FilenameReplaceChar);
                if (newName != name)
                {
                    String newPath = Path.GetDirectoryName(directory) + "\\" + series.SeriesName;
                    if (IOFunctions.MergeDirectories(directory, newPath, options.overwrite))
            ...
            if (options.GetSeriesPosters)
                SaveImage(directory, "folder.jpg", posters, 0, -1, Providers.TVShowPosterType.Poster, level);

            if (options.DownloadAllPosters)
            {
                Message("Downloading All Posters", MediaScoutMessage.MessageType.Task, level);
                if (posters == null)
                    posters = tvdb.GetPosters(series.ID, TVShowPosterType.Poster, -1);
                if (posters != null)
                {
                    int i = 0;
                    foreach (Posters p in posters)
                    {
                        SaveImage(directory, "folder" + i + ".jpg", posters, i, -1, Providers.TVShowPosterType.Poster, level + 1);
                        i++;
                    }
                }
                else
                    Message("No Posters Found", MediaScoutMessage.MessageType.TaskResult, level);
            ...
            if (options.GetSeriesPosters)
            {
                if (options.SaveXBMCMeta)
                    SaveImage(directory, "fanart.jpg", backdrops, 0, -1, Providers.TVShowPosterType.Backdrop, level);
                if (options.SaveMyMoviesMeta)
                    SaveImage(directory, "backdrop.jpg", backdrops, 0, -1, Providers.TVShowPosterType.Backdrop, level);
            }
            ...
            if (options.DownloadAllBackdrops)
            {
                ...
                if(backdrops == null)
                    backdrops = tvdb.GetPosters(series.id, MediaScout.Providers.TVShowPosterType.Backdrop, -1);
                if (backdrops != null)
                {
                    int i = 0;
                    ...
                    {
                        if (options.SaveXBMCMeta)
                            SaveImage(directory, "fanart" + i + ".jpg", backdrops, i, -1, Providers.TVShowPosterType.Backdrop, level + 1);
                        if (options.SaveMyMoviesMeta)
                            SaveImage(directory, "backdrop" + i + ".jpg", backdrops, i, -1, Providers.TVShowPosterType.Backdrop, level + 1);
                        i++;
                    }
                }
                else
                    Message("No Backdrops Found", MediaScoutMessage.MessageType.TaskResult, level);
            }
            ...
            if (options.GetSeriesPosters)
                SaveImage(directory, "banner.jpg", banners, 0, -1, Providers.TVShowPosterType.Banner, level);

            if (options.DownloadAllBanners)
            {
                Message("Downloading All Banners ", MediaScoutMessage.MessageType.Task, level);
                if(banners == null)
                    banners = tvdb.GetPosters(series.id, MediaScout.Providers.TVShowPosterType.Banner, -1);
                if (banners != null)
                {
                    int i = 0;
                    foreach (Posters p in banners)
                    {
                        SaveImage(directory, "banner" + i + ".jpg", banners, i, -1, Providers.TVShowPosterType.Banner, level + 1);
                        i++;
                    }
                }
                else
                    Message("No Banners Found", MediaScoutMessage.MessageType.TaskResult, level);
            }
                ...
                if (options.SaveXBMCMeta)
                {
                    ...
                    if(series.Persons.Count != 0)
                    {
                        ...
                        if (!Directory.Exists(ActorsDir))
                            IOFunctions.CreateHiddenFolder(ActorsDir);
                ...
                if (series.Seasons.ContainsKey(seasonNum))
                {
                    if (diSeason.Name != options.SpecialsFolderName)
                    {
                        ...
                        if (options.RenameFiles)
                        {
                            ...
                            {
                                if (IOFunctions.MergeDirectories(diSeason.FullName, newPath, options.overwrite))
                                    name = "d:" + newName;
                                else
                                    name = newName;
                            }
        ...
        public void ProcessSeason(String directory, String seasonFldr, int seasonNum, int level)
        {
            ...
            if (options.GetSeasonPosters)
                SaveImage(directory + "\\" + seasonFldr, "folder.jpg", posters, 0, seasonNum, MediaScout.Providers.TVShowPosterType.Season_Poster, level);
            ...
            if (options.DownloadAllSeasonPosters)
            {
                ...
                if(posters == null)
                    posters = tvdb.GetPosters(series.id, MediaScout.Providers.TVShowPosterType.Season_Poster, seasonNum);
                if (posters != null)
                {
                    int i = 0;
                    foreach (Posters p in posters)
                    {
                        SaveImage(directory + "\\" + seasonFldr, "folder" + i + ".jpg", posters, i, seasonNum, MediaScout.Providers.TVShowPosterType.Season_Poster, level + 1);
                        i++;
                    }
                }
                else
            ...
            {
                if (options.SaveXBMCMeta)
                    SaveImage(directory + "\\" + seasonFldr, "fanart.jpg", backdrops, 0, seasonNum, MediaScout.Providers.TVShowPosterType.Season_Backdrop, level);
                if (options.SaveMyMoviesMeta)
                    SaveImage(directory + "\\" + seasonFldr, "backdrop.jpg", backdrops, 0, seasonNum, MediaScout.Providers.TVShowPosterType.Season_Backdrop, level);
            }

            if (options.DownloadAllSeasonBackdrops)
            {

                Message("Downloading All " + seasonFldr + " Backdrops", MediaScoutMessage.MessageType.Task, level + 1);
                if(backdrops == null)
                    backdrops = tvdb.GetPosters(series.id, MediaScout.Providers.TVShowPosterType.Season_Backdrop, seasonNum);
                if (backdrops != null)
                {
                    int i = 0;
                    foreach (Posters p in backdrops)
                    {
                        if (options.SaveXBMCMeta)
                            SaveImage(directory + "\\" + seasonFldr, "fanart" + i + ".jpg", backdrops, i, seasonNum, MediaScout.Providers.TVShowPosterType.Season_Backdrop, level + 1);
                        if (options.SaveMyMoviesMeta)
                            SaveImage(directory + "\\" + seasonFldr, "backdrop" + i + ".jpg", backdrops, i, seasonNum, MediaScout.Providers.TVShowPosterType.Season_Backdrop, level + 1);
                        i++;
                    }
                }
                else
        ...
        public String ProcessEpisode(String ShowDirectory, FileInfo fi, int seasonNum, bool IsInsideShowDir, int level)
        {
            ...
            if (options.MoveFiles)
            {
                if (!IsInsideShowDir)
                {
                    String newDirPath = ShowDirectory + "\\" + IOFunctions.GetValidName(series.SeriesName, options.FilenameReplaceChar);
            ...
            try
            {
                EpisodeInfo ei = GetID.GetSeasonAndEpisodeIDFromFile(fi.Name);
        ...
        {            
            ...
            if (options.RenameFiles)
            {
                ...
                String newName = String.Format(options.RenameFormat, series.SeriesName, seasonID.ToString().PadLeft(options.SeasonNumZeroPadding, '0'), episode.EpisodeName.Trim(), episodeID.ToString().PadLeft(options.EpisodeNumZeroPadding, '0'));
                newName = IOFunctions.GetValidName(newName, options.FilenameReplaceChar);
            ...
            {                
                if (!String.IsNullOrEmpty(episode.PosterUrl))
                {
                    ...
                    try
                    {
                        ...
                        if (options.SaveMyMoviesMeta)
                        {
                            ...
                            String filename = episode.GetMyMoviesThumbFile(file.DirectoryName);
                            if (!File.Exists(filename))
                            {
                                p.SavePoster(filename);
                                Message("Done", MediaScoutMessage.MessageType.TaskResult, level);
                ...
                else
                {
                    ...
                    if (options.SaveMyMoviesMeta)
                    {
                        ...
                        {
                            VideoInfo.SaveThumb(file.FullName, Filename, 0.25);
                            episode.PosterName = episode.GetMyMoviesThumbFilename();
                            Message("Done", MediaScoutMessage.MessageType.TaskResult, level);
                        }
                        else
                            Message("Already Exists, skipping", MediaScoutMessage.MessageType.TaskResult, level);
        ...
        public FileInfo MoveFileToAppropriateFolder(String ShowDirectory, FileInfo fi, int seasonID, int episodeID, int level)
        {
            ...
            if (seasonID == -1 || episodeID == -1)
                ei = GetID.GetSeasonAndEpisodeIDFromFile(fi.Name);
        ...
        private FileInfo MoveFile(String src, String dest, int level, bool IsRenameOperation)
        {
            ...
            try
            {
                ...
                fi =IOFunctions.MoveFile(fi, dest, options.overwrite);
            }
            catch (Exception ex)
            {
                Message(ex.Message, MediaScoutMessage.MessageType.TaskError, level);
        ...
        private void SaveImage(String directory, String filename, Posters[] images, int index, int SeasonNum, Providers.TVShowPosterType ptype, int level)
        {
            Message("Saving " + ptype.ToString().Replace("_", (SeasonNum != -1) ? " " + SeasonNum + " " : " ") + " as " + filename, MediaScoutMessage.MessageType.Task, level);
            if (!File.Exists(directory + "\\" + filename) || options.ForceUpdate == true)
            {
                try
                {
                    ...
                    if (images != null)
                    {
                        ...
                        Message("No " + ptype.ToString().Replace("_", (SeasonNum != -1) ? " " + SeasonNum + " " : " ") + "s Found", MediaScoutMessage.MessageType.TaskError, level);
                }
                catch (Exception ex)
                {
                    Message(ex.Message, MediaScoutMessage.MessageType.TaskError, level);
