    ...
    public class TheMovieDBProvider : IMovieMetadataProvider
    {
        ...
        public MovieXML[] Search(string MovieName)
        {
            ...
            try
            {
                ...
                {
                    if (Message != null)
                        Message(movies.Count + " Found", MediaScoutMessage.MessageType.TaskResult, level);
        ...
        public MovieXML Get(string MovieID, DateTime dtCacheTime)
        {
            ...
            try
            {
                ...
                //Get the genres
                XmlNodeList nlGenres = nlMovie[0].SelectNodes("./categories/category");
                foreach (XmlNode x in nlGenres)
                    m.Genres.Add(new Genre() { name = x.Attributes["name"].Value });

                XmlNodeList nlStudios = nlMovie[0].SelectNodes("./studios/studio");
                foreach (XmlNode x in nlStudios)
                    m.Studios.Add(new Studio() { name = x.Attributes["name"].Value });

                //Get the actors/directors/etc
                XmlNodeList nlActors = nlMovie[0].SelectNodes("./cast/person");
        ...
        public List<Posters> GetPersonImage(String PersonID)
        {
            ...
            try
            {
                ...
                String imageThumb = "./images/image[@size='thumb']";
