...
namespace MediaScout.Providers
{
    public class TheTVDBProvider : ITVMetadataProvider
    {
        ...
        /// Api to Download Clearart
        /// http://www.lockstockmods.net/logos/getlogo.php?id=thetvdb_id
        /// <summary>
        ...
        public TVShowXML[] Search(String SeriesName, String Language)
        {
            ...
            try
            {
                ...
                for (int i = 0; i < xnl.Count; i++)
                {
                    ...
                    //Posters[] posters = GetPosters(t.id, TVShowPosterType.Poster, -1);
                    //if (posters != null && posters.Length > 0)
                    //    t.PosterThumb = posters[0].Thumb;
                    //else
                    //    t.PosterThumb = "";
                ...
                {
                    if (Message != null)
                        Message(tvshows.Count + " Found", MediaScoutMessage.MessageType.TaskResult, level);
                    return tvshows.ToArray();
                }
                else
                {
                    if (Message != null)
                        Message("Nothing Found", MediaScoutMessage.MessageType.TaskError, level);
                }
            }
            catch (Exception ex)
            {
            ...
            try
            {
                ...
                foreach (XmlNode x in nodeList)
                {
                    ...
                    if (!s.Seasons[SeasonNumber].Episodes.ContainsKey(EpisodeNumber))
                    {
                        ...
                        ep.LastUpdated = x["lastupdated"].InnerText;
                        ep.SeriesID = x["seriesid"].InnerText;
        ...
        public EpisodeXML GetEpisode(String TVShowID, String SeasonID, String EpisodeID)
        {
            ...
            try
            {
                ...
                nodeList = node.SelectNodes("/Data/Episode");
                e.EpisodeID = e.ID = nodeList[0].SelectSingleNode("id").InnerText; ;
        ...
        public Posters[] GetPosters(String TVShowID, TVShowPosterType type, int seasonNum)
        {
            ...
            try
            {
                ...
                foreach (XmlNode x in xnl)
                {
                    if (x.SelectSingleNode("BannerType").InnerText == t)
                    {
                        ...
                        {
                            if (x.SelectSingleNode("Season").InnerText != seasonNum.ToString())
                                continue;
                        }
            ...
            {
                if (Message != null)
                    Message(ex.Message, MediaScoutMessage.MessageType.TaskError, (seasonNum!=-1)? level + 1 : level);
            }
