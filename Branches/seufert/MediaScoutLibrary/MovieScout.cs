...
namespace MediaScout
{
    public class MovieScout
    {
        ...
        {
            String NewDirName = String.Format(options.DirRenameFormat, m.Title, m.ProductionYear);
            return IOFunctions.GetValidName(NewDirName, options.FilenameReplaceChar);
        }
        public String ProcessDirectory(String directory)
        {
            ...
            {
                String NewName = GetMovieDirName();
                if (name != NewName)
                {
                    String NewPath = Path.GetDirectoryName(directory) + "\\" + NewName;
            ...
            if (options.GetMoviePosters)
                SaveImage(directory, "folder.jpg", posters, 0, Providers.MoviePosterType.Poster, level);

            if (options.DownloadAllPosters)
            {
                Message("Downloading All Posters", MediaScoutMessage.MessageType.Task, level);
                if(posters == null)
                    posters = tmdb.GetPosters(m.ID, MoviePosterType.Poster);
                if (posters != null)
                {
                    int i = 0;
                    ...
                    Message("No Posters Found", MediaScoutMessage.MessageType.TaskError, level);
            }
            ...
            if (options.GetMoviePosters)
            {
                if(options.SaveXBMCMeta)
                    SaveImage(directory, "fanart.jpg", backdrops, 0, MoviePosterType.Backdrop, level);
                if (options.SaveMyMoviesMeta)
                    SaveImage(directory, "backdrop.jpg", backdrops, 0, MoviePosterType.Backdrop, level);
            }
            ...
            if (options.DownloadAllBackdrops)
            {
                Message("Downloading All Backdrops", MediaScoutMessage.MessageType.Task, level);                
                 if(backdrops == null)
                    backdrops = tmdb.GetPosters(m.ID, MoviePosterType.Backdrop);
                 if (backdrops != null)
                 {
                     int i = 0;
                     ...
                     Message("No Backdrops Found", MediaScoutMessage.MessageType.TaskError, level);
                ...
                if (options.SaveXBMCMeta)
                {
                    ...
                    if (m.Persons.Count != 0)
                    {
                        ...
                        foreach (Person p in m.Persons)
                        {
                            if (p.Type == "Actor")
                            {
                                List<Posters> Thumb = tmdb.GetPersonImage(p.ID);
                                if (Thumb != null && Thumb.Count > 0)
                                    p.Thumb = Thumb[0].Poster;
        ...
        public String ProcessFile(String MovieDirectory, FileInfo fi, bool IsInsideMovieFolder, int level)
        {
            ...
            if (options.RenameFiles)
            {
                if (type == MovieType.Single_File_Rip || type == MovieType.Multi_File_Rip || type == MovieType.Trailer || type == MovieType.Sample)
                {
                    ...
                    FileInfo OldFile = new FileInfo(fi.FullName);
                    String newName = String.Format(options.FileRenameFormat, m.Title, m.ProductionYear);
            ...
            {
                if (options.SaveXBMCMeta)
                    SaveImage(fi.DirectoryName, m.GetXBMCThumbFilename(fi.Name.Replace(fi.Extension, "")), posters, 0, MoviePosterType.File_Poster, level + 1);
            }
            #endregion
            ...
            {
                if (options.SaveXBMCMeta)
                    SaveImage(fi.DirectoryName, m.GetXBMCBackdropFilename(fi.Name.Replace(fi.Extension, "")), backdrops, 0, MoviePosterType.File_Backdrop, level + 1);
            }
